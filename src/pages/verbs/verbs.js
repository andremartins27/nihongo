...
function Verbs() {
  return (<div className="ui container" style={{padding: '1em'}}>
    <div>
    ...
    <table className="ui selectable celled table">
      ...
      <tbody>
        {VERB_DB.sort((verb1, verb2) => { return verb1.dictionary.english.localeCompare(verb2.dictionary.english)}).map(
          /** @type {Verb} */ (verb) => {
            let dictionary = verb.dictionary;
            let present = Conjugation.present(verb);
            let presentNegative = Conjugation.presentNegative(verb);
            return (<tr key={dictionary.english} className={getColor(verb.type)}>
